...
Now we need to pick the solution that minimizes $\mathbf{u}_2$. We can solve it as an optimization (by thinking about the derivatives of the objective/cost function), but it can also be solved as a projection.

\bigskip

Let us define projector $\mathbf{P} = \mathbf{B}_1 \mathbf{B}^+_1$. \textcolor{mydarkgreen}{We can prove that $\mathbf{P} \mathbf{B}_2 \mathbf{u}^*_2 = \mathbf{0}$}. Assume $\mathbf{P} \mathbf{B}_2 \mathbf{u}^*_2 = \mathbf{a}$, then $\mathbf{u}^*_2 =  \mathbf{u}^0_2 + \mathbf{u}^a_2$, where $\mathbf{u}^0_2$ is in the null space of $\mathbf{P} \mathbf{B}_2$ and $\mathbf{u}^a_2$ is in the row space of $\mathbf{P} \mathbf{B}_2$, or equivalently $\mathbf{P} \mathbf{B}_2 \mathbf{u}^0_2 = \mathbf{0}$ and $\mathbf{P} \mathbf{B}_2 \mathbf{u}^a_2 = \mathbf{a}$. This gives us solution:
...
But since $\mathbf{a} \in \text{span}(\mathbf{B}_1) = \text{span}(\mathbf{P})$ we can also provide an alternative solution:
...
Since $\mathbf{u}^0_2$ and $\mathbf{u}^a_2$ belong to orthogonal subspaces, $||\mathbf{u}^0_2 + \mathbf{u}^a_2|| \geq ||\mathbf{u}^0_2||$. Therefore the alternative solution provides a smaller norm value for $\mathbf{u}_2$, hence $\mathbf{u}^a_2 = \mathbf{0}$ and $\mathbf{a} = \mathbf{0}$ and $\mathbf{P} \mathbf{B}_2 \mathbf{u}^*_2 = \mathbf{0}$.

\bigskip

Since $\mathbf{P} \mathbf{B}_2 \mathbf{u}^*_2 = \mathbf{0}$ and $\mathbf{P} \mathbf{B}_1 = \mathbf{B}_1$ we can observe that $(\mathbf{I} - \mathbf{P}) \mathbf{B}_2 \mathbf{u}^*_2 = \mathbf{B}_2\mathbf{u}^*_2$ and $(\mathbf{I} - \mathbf{P}) \mathbf{B}_1 = \mathbf{0}$.
...
Since $(\mathbf{I} - \mathbf{P}) \mathbf{B}_2 \mathbf{u}^*_2 = \mathbf{B}_2\mathbf{u}^*_2$, as a constraint on $\mathbf{u}^*_2$ equation \eqref{eq:projected} is equivalent to \eqref{eq:task_u1u2}. Eq. \eqref{eq:projected} has exact solution:
